...
import { TMDB_BASE_URL, API_KEY } from "../utils/constants"; // Import TMDB base URL and API key
...
function MyBasketPage() {
  ...
  const generateShareableLink = (basket) => {
    try {
      ...
      const link = TMDB_BASE_URL + path + queryString;
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-4">
          {basketMovies.map((movie) => (
            <div key={movie.id} className="relative">
              <img
                // src={`${TMDB_BASE_URL}/w500${movie.poster_path}?api_key=${API_KEY}`}
